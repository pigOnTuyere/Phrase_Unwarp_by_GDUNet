...
class Outconv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        # 添加的模块
        # self.rlu_out = nn.LeakyReLU(inplace=True)
    def forward(self, x):
        x1 = self.conv(x)
        # 添加的模块
        # x1 = self.rlu_out(x1)
...
class UNet(nn.Module):
    def __init__(self,input_chanel=1,output_chanel=2):
        super(UNet, self).__init__()
        self.down1 = DownB(input_chanel, 64)
        ...
        self.up4 = UpB(128, 64)
        self.outc = Outconv(64, output_chanel)
    def forward(self, x):
        x1, x1_ = self.down1(x)
...
if __name__ == "__main__":
    ...
    import  torch
    model =UNet(input_chanel=3,output_chanel=2).to('cuda')
    ...
    # 前向传播前打印 GPU 内存占用
    print(f"GPU memory allocated before forward pass: {torch.cuda.memory_allocated() / 1024 ** 2:.2f} MB")

    # 前向传播
    output = model(x)

    # 前向传播后打印 GPU 内存占用
    print(f"GPU memory allocated after forward pass: {torch.cuda.memory_allocated() / 1024 ** 2:.2f} MB")

    # 清空缓存
    torch.cuda.empty_cache()

    # 打印清空缓存后的 GPU 内存占用
    print(f"GPU memory allocated after clearing cache: {torch.cuda.memory_allocated() / 1024 ** 2:.2f} MB")